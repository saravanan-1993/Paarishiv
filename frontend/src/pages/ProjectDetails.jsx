...
const DPRStatusDropdown = ({ dpr, onStatusChange }) => {
    const [isOpen, setIsOpen] = useState(false);
    // Single-stage DPR flow: Pending → Approved (or Rejected). Legacy
    // intermediate values (Reviewed / Coordinator Approved / Dept Approved)
    // are no longer offered to new approvers, but old DPRs already in those
    // states are still finalisable from Approvals or SiteReports.
    const options = ['Pending', 'Approved', 'Rejected'];

    const getColors = (status) => {
        if (status === 'Approved') return { bg: '#ECFDF5', text: '#059669', border: '#A7F3D0', dot: '#10B981' };
        if (status === 'Reviewed' || status === 'Coordinator Approved' || status === 'Dept Approved') return { bg: '#EFF6FF', text: '#2563EB', border: '#BFDBFE', dot: '#3B82F6' };
