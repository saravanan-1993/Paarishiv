...
import {
    ...
    IndianRupee, Loader2, Calendar, Building2, X
} from 'lucide-react';
...
const Quotations = () => {
    ...
    const handleDelete = async (q) => {
        if (!(await confirm({ title: 'Delete Quotation', message: `Delete quotation ${q.quotation_no}?`, confirmText: 'Delete', danger: true }))) return;
        try {
            await quotationAPI.delete(q.id);
            await loadData();
            toast.success(`Deleted quotation ${q.quotation_no}`);
        } catch (err) {
            console.error(err);
    ...
    const doSendEmail = async (q, email) => {
        setSendingId(q.id);
        try {
            ...
            toast.success(`Quotation ${q.quotation_no} emailed to ${email} with PDF attached.`);
            await loadData();
        } catch (err) {
            console.error(err);
            toast.error(err.response?.data?.detail || 'Failed to send email. Check SMTP settings.');
        }
        setSendingId(null);
    };
    ...
    const handleSendEmail = async (q) => {
        ...
        if (!existing) {
            setEmailPrompt({
                ...
                message: `No email on file for ${q.client_name || 'this client'}. Enter an email to send the quotation.`,
                placeholder: 'client@example.com',
                confirmText: 'Send'
            });
            return;
        }
        const proceed = await confirm({ title: 'Send Quotation', message: `Send quotation ${q.quotation_no} to ${existing} (with PDF attachment)?`, confirmText: 'Send' });
        if (!proceed) return;
        await doSendEmail(q, existing);
    };
        ...
        <div style={{ padding: '24px' }}>
            ...
            <div style={{ backgroundColor: 'white', padding: 16, borderRadius: 12, border: '1px solid #E2E8F0', marginBottom: 16, display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                <div style={{ flex: 1, minWidth: 260, position: 'relative' }}>
                    ...
                    <input
                        ...
                        style={{ width: '100%', padding: '10px 36px 10px 36px', borderRadius: 8, border: '1px solid #E2E8F0', fontSize: 14, outline: 'none' }}
                    />
                    {search && (
            ...
            <PromptModal
                ...
                onSubmit={(val) => {
                    ...
                    if (!email) return;
                    doSendEmail(emailPrompt.quotation, email);
                }}
                title={emailPrompt?.title}
                message={emailPrompt?.message}
                placeholder={emailPrompt?.placeholder}
                confirmText={emailPrompt?.confirmText}
                required
                multiline={false}
            />
        </div>
